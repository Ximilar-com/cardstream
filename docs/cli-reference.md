...
| `--price-stats`, `--price_stats` | off | ask the id endpoint for market price statistics with every match — USD median, range and latest sale, shown on the page and in the terminal. tcg, sport and comics only (slab has none); off by default, since the extra data is not documented as free. The web UI's settings dialog toggles it live. |
